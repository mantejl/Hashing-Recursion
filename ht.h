...
public:
    ...
    HASH_INDEX_T next()
    {
        ...
        temporary = this->start_ + this->numProbes_ * dhstep_ * one;
...
private:
    ...
    double sizingFactor, verification;
...
void HashTable<K, V, Prober, Hash, KEqual>::resize()
{
    ...
    for (int i = 0; i < resizedTable.size(); i++)
    {
        if (resizedTable[i])
        {
            ...
            {
                tableSize++;
                locationCur = this->probe(resizedTable[i]->item.first);
                table_[locationCur] = resizedTable[i];
            }
